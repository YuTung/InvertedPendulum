...
boolean plotPitch = true , plotRoll = true , plotYaw = true;
...
void plotBackground(){  
 ...
 // sensor used
  ...
  textSize(20);  fill(0,140,50);  text(trim(", roll:"),100,520);
 ...
 //plot pitch,roll,yaw 
   ...
   if( mouseX>=108 && mouseX<=148 && mouseY>=503 && mouseY<=526 ){
       noFill();  stroke(0);  rect(108,503,40,23);
   }else{
      if(plotRoll){
        noFill();  stroke(0,140,50);  rect(108,503,40,23);
      }
   }
...
void plotSensorData(){
 ...
 //roll 
  if(plotRoll){
    quad_roll[quad_roll.length-1] = roll;
    noFill();
    stroke(0,140,50);
    beginShape();
      for(int i = 0; i<quad_roll.length;i++)
        vertex(i,quad_roll[i]+400);
    endShape();
  }else{
   for(int i = 0 ; i < 750 ; i ++)
    quad_roll[i] = 0;
  ...
  if(!stopORrunPlot){//false->run , true:stop
    for(int i = 1; i<quad_roll.length;i++)
      quad_roll[i-1] = quad_roll[i];
  }
  //yaw  
  if(plotYaw){
...
void plotText(){
 // mode
  if(mode == 1){
   textSize(30);  fill(0,0,0);  text(trim("IDLE"), 320 ,50); 
  }else if(mode == 2){
   textSize(30);  fill(0,0,0);  text(trim("FLYING"), 320 ,50);
  }
